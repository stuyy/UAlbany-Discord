...
{
  var gallery_search_url = 'https://api.imgur.com/3/gallery/search/?q=' + query + '&client_id=' + config.imgur_client_id;
  request(gallery_search_url, function(err, res, body){
    try
    {
      ...
      message.channel.send({files: [new Discord.Attachment(queryData.data[random].link), query]});
    }
    catch(error)
    {
      console.log(error);
